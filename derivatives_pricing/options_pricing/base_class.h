...
public:
  ...
  explicit Option(double strike, double maturity, double underlying_price, double volatility, double risk_free_rate, option_type type)
  : _strike(strike), _maturity(maturity), _underlying_price(underlying_price), _volatility(volatility), _risk_free_rate(risk_free_rate), _option_type(type) {}

  //Setter methods for all input variables
  void set_strike(double value) {_strike = value;}
  void set_maturity(double value) {_maturity = value;}
  ...
  double get_maturity() const { return _maturity; }
